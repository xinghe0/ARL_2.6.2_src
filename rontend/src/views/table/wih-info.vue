<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input
        v-model="localListQuery.record_type"
        placeholder="类型"
        style="width: 200px;"
        class="filter-item"
    ...
    >
      ...
      <el-table-column label="类型" min-width="210px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.record_type }}</span>
        </template>
      </el-table-column>
      <el-table-column label="内容" width="460px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.content }}</span>
        </template>
      </el-table-column>
      <el-table-column label="站点" width="250px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.site }}</span>
        </template>
      </el-table-column>
      <el-table-column label="JS连接" width="520px" align="center">
        <template slot-scope="{row}">
          <span>{{ decodeUnicode(row.source) }}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  created() {
    ...
    this.$set(this.localListQuery, 'record_type', '')
    this.$set(this.localListQuery, 'tite', '')
  },
  methods: {
    decodeUnicode(data) {
    ...
    getList() {
      ...
      wihList(this.listQuery).then(response => {
    ...
    searchgetList() {
      this.listLoading = true
      wihList(this.localListQuery).then(response => {
        this.list = response.items
        this.total = response.total
