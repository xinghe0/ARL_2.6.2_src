<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.ip')" min-width="160px">
        <template slot-scope="{ row }">
          <a class="link-type" :href="'https://'+row.ip+':'+row.port" target="_blank">{{ 'https://'+row.ip+':'+row.port  }}</a>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.cert')" width="1300px" >
        <template slot-scope="{row}">
          主题名称:&nbsp;&nbsp;<span>{{ row.cert.subject_dn }}</span><br>
          签发者名称:&nbsp;&nbsp;<span>{{ row.cert.issuer.common_name }}</span><br>
          序列号:&nbsp;&nbsp;<span>{{ row.cert.serial_number }}</span><br>
          SHA-256 :&nbsp;&nbsp;<span>{{ row.cert.fingerprint.sha256 }}</span><br>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    searchgetList() {
      ...
      sslList(this.localListQuery).then(response => {
    ...
    getList() {
      this.listLoading = true
      sslList(this.listQuery).then(response => {
        this.list = response.items
        this.total = response.total
