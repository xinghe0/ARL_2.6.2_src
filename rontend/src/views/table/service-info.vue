<template xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">
  <div class="app-container">
    <div class="filter-container">
      <el-input
        v-model="localListQuery.service_name"
    ...
    >
      ...
      <el-table-column :label="$t('table.service')" min-width="160px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.service_name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.info')" width="700px" align="center">
        <template slot-scope="{row}">
          <div class="scrollable">
          <span>{{ row.service_info.map(item => item.ip).join(',') }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.product')" width="520px" align="center">
        <template slot-scope="{row}">
          <span>{{row.service_info.product? row.service_info.map(item => item.product).join(','):'-' }}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  created() {
    ...
    this.$set(this.localListQuery, 'service_name', '')
  },
  methods: {
    getList() {
      this.listLoading = true
      serList(this.listQuery).then(response => {
    ...
    searchgetList() {
      this.listLoading = true
      serList(this.localListQuery).then(response => {
        this.list = response.items
        this.total = response.total
