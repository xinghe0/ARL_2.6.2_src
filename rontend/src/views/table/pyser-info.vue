<template xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.protocol')" min-width="160px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.protocol }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.target')" width="500px" align="center">
        <template slot-scope="{row}">
          <span>{{row.target }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.ip')" width="300px" align="center">
        <template slot-scope="{row}">
            <span>{{ row.ip }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.port')" width="300px" align="center">
        <template slot-scope="{row}">
          <span>{{row.port }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.$set(this.localListQuery, 'scheme', '')
    this.$set(this.localListQuery, 'host', '')
  },
  methods: {
    getList() {
      this.listLoading = true
      pyserList(this.listQuery).then(response => {
    ...
    searchgetList() {
      this.listLoading = true
      pyserList(this.localListQuery).then(response => {
        this.list = response.items
        this.total = response.total
