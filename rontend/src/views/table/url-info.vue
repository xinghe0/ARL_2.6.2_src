<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.url')" min-width="260px">
        <template slot-scope="{ row }">
          <a class="link-type" :href="row.url" target="_blank">{{ row.url }}</a>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.title')" width="230px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column :label="$t('table.source')" width="220px" align="center">
        <template slot-scope="{row}">
...
export default {
  ...
  methods: {
    ...
    searchgetList() {
      this.listLoading = true
      urlList(this.localListQuery).then(response => {
        this.list = response.items
        this.total = response.total
