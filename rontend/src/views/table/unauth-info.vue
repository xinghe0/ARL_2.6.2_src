<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <el-input
        v-model="localListQuery.status_code"
    ...
    >
      ...
      <el-table-column :label="$t('table.url')" min-width="410px">
        <template slot-scope="{ row }">
          <a class="link-type" :href="row.url" target="_blank">{{ row.url }}</a>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.content_length')" width="160px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.len }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="内容" width="420px" align="center">
        <template slot-scope="{row}">
          <div class="scrollable">
          <span>{{ decodeUnicode(row.content) }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="js链接" width="270px" align="center">
        <template slot-scope="{row}">
            <a class="link-type" :href="row.jslink" target="_blank">点击查看</a>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  created() {
    ...
    this.$set(this.localListQuery, 'status_code', '')
  },
  methods: {
    decodeUnicode(data) {
    ...
    getList() {
      ...
      unauthList(this.listQuery).then(response => {
    ...
    searchgetList() {
      this.listLoading = true
      unauthList(this.localListQuery).then(response => {
        this.list = response.items
        this.total = response.total
