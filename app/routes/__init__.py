...
import time
from ..utils.arl import task_statistic
...
class ARLResource(Resource):
    ...
    def build_data_task(self, args=None, collection=None):
        ...
        for document in result:
            statistic = task_statistic(str(document['_id']))
            document['statistic'] = statistic
            modified_result.append(document)
        count = conn(collection).count(query)
        items = self.build_return_items(modified_result)
...
from .unauth import ns as unauth_ns
from .xray_result import ns as xray_result
from .src import ns as src_ns