...
from app import arlmodules
from app.arlmodules import ScanPortType, DomainDictType, CollectSource, TaskStatus
...
from oneforall_api import oneforall
    ...
    def run(self):
        ...
        for domain in self.resolver_map:
            ...
            if ips:
                ...
                self.domain_info_list.append(arlmodules.DomainInfo(**item))
...
class ScanPort(object):
    ...
    def run(self):
        ...
        for result in ip_port_result:
            ...
            for port_info in result["port_info"]:
                port_info_obj_list.append(arlmodules.PortInfo(**port_info))

            result["port_info"] = port_info_obj_list

            ip_info_obj.append(arlmodules.IPInfo(**result))
    ...
    def build_fake_cdn_ip_info(self):
        ...
        fake_80_port = arlmodules.PortInfo(**map_80_port)
        ...
        fake_443_port = arlmodules.PortInfo(**map_443_port)
        ...
        for ip in self.ip_cdn_map:
            ...
            ret.append(arlmodules.IPInfo(**item))
...
class DomainTask(CommonTask):
    ...
    def build_domain_info(self, domains):
        ...
        for item in domains:
            ...
            fake_info = arlmodules.DomainInfo(**fake)
    ...
    def save_ip_info(self):
        ...
        for ip in self.ipv4_map:
            ...
            info_obj = arlmodules.IPInfo(**data)
    ...
    def build_single_domain_info(self, domain):
        ...
        return arlmodules.DomainInfo(**item)
    ...
    def domain_oneforall(self):
        if not self.options.get("domain_oneforall"):
            return

        if "{fuzz}" in self.base_domain:
            return
        logger.info("start domain_oneforall task")
        self.update_task_field("status", "domain_oneforall")
        search_engines_urls = oneforall(self.base_domain)
        t1 = time.time()

        urls = set()  # 保存通过oneforall获取到的URL
        domains = set()
        for url in search_engines_urls:
            parse = urlparse(url)
            netloc = parse.netloc
        ...
        if len(domains) > 0:
            ...
            if self.task_tag == "task":
                domain_info_list = self.clear_domain_info_by_record(domain_info_list)
                self.save_domain_info_list(domain_info_list, source=CollectSource.DOMAIN_ONEFORALL)
            self.domain_info_list.extend(domain_info_list)
        elapse = time.time() - t1
        self.update_services("domain_oneforall", elapse)

        logger.info("domain_oneforall {}, result domain:{} url:{}".format(self.base_domain,
                                                                          len(domain_info_list),
                                                                          len(urls)))
        ...
        if len(urls) > 0:
            page_map = services.page_fetch(urls)
            for url in page_map:
                item = build_url_item(url, self.task_id, source=CollectSource.DOMAIN_ONEFORALL)
                item.update(page_map[url])
                utils.conn_db('url').insert_one(item)
