...
from app.arlmodules import CollectSource, WebSiteFetchStatus, WebSiteFetchOption
from app.services.nuclei_scan import nuclei_scan
from app.services import run_risk_cruising, BaseUpdateTask
from app.tools.xray_scan.xray_scan import xray_scans
from app.utils.push import feishu_send
...
class WebSiteFetch(object):
    ...
    def xray_scan(self):
        logger.info("start xray_scan， poc_sites:{}".format(len(self.poc_sites)))
        scan_results = xray_scans(list(self.poc_sites))
        if scan_results:
            for item in scan_results:
                item["task_id"] = self.task_id
                item["save_date"] = utils.curr_date()
                utils.conn_db('xray_result').insert_one(item)
            logger.info("end xray_scan， result:{}".format(len(scan_results)))
        else:
            logger.info("end xray_scan， result: 0")

    def unauth_scan(self):
        file_name = utils.random_choices()
        logger.info("start unauth_scan， poc_sites:{}".format(len(self.poc_sites)))
        result =  Packerzuuer_scan(list(self.poc_sites), file_name)
        scan_results = unauth_run(list(self.poc_sites))
        if len(result) != 0:
            result = result["data"]
            wrapped_str = '[' + result + ']'
            result = ast.literal_eval(wrapped_str)
            scan_results1 = result
        else:
            scan_results1 = []
        scan_results = scan_results1 + scan_results
        unique_results = []
        seen_urls = set()
        ...
        for result in scan_results:
            ...
            if url not in seen_urls:
                ...
                seen_urls.add(url)
        scan_results = unique_results
        if scan_results:
            for item in scan_results:
                item["task_id"] = self.task_id
                item["save_date"] = utils.curr_date()
                utils.conn_db('unauth').insert_one(item)
            logger.info("end unauth_scan， result:{}".format(len(scan_results)))
        else:
            logger.info("end unauth_scan， result: 0")
    ...
    def run(self):
        ...
        """ 对站点运行未授权扫描 """
        if self.options.get(WebSiteFetchOption.UNAUTH_SCAN):
            self.run_func(WebSiteFetchStatus.UNAUTH_SCAN, self.unauth_scan)
