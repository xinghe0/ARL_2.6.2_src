...
class DatabaseType():
        ...
        for url, text in res.items():
            ...
            try:
                for ext in black_ext:
                    if ("<html" not in text) and ("PNG" not in text) and (len(text) != 0) and (
                            url.split("/")[-1] != "favicon.ico") \
    ...
    def insertCorsInfoIntoDB(self, request_b, response_h):
        ...
        response_h = "Access-Control-Allow-Origin: " + response_h[
            'Access-Control-Allow-Origin'] + ", Access-Control-Allow-Credentials: " + response_h[
