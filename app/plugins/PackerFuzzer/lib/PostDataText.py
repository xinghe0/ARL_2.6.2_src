...
class PostsDataText(object):
    ...
    def check(self, url, data, jsondata):
        ...
        try:
            ...
            if sslFlag == 1:
                text = str(requests.post(url, headers=headers, timeout=6, data=data, proxies=self.proxy_data,
                                         verify=False).text)  # 正常的返回code是int类型
                code = str(requests.post(url, headers=headers, timeout=6, data=data, proxies=self.proxy_data,
                                         verify=False).status_code)
            else:
                text = str(requests.post(url, headers=headers, timeout=6, data=data, proxies=self.proxy_data).text)
                code = str(
                    requests.post(url, headers=headers, timeout=6, data=data, proxies=self.proxy_data).status_code)
            ...
            while (code == "415" or code == "401"):
                ...
                if tag == 1:
                    ...
                    if code == "200":
                        if sslFlag == 1:
                            text = str(
                                requests.post(url, headers=header, timeout=6, data=jsondata, proxies=self.proxy_data,
                                              verify=False).text)  # 正常的返回code是int类型
                        else:
                            text = str(requests.post(url, headers=header, timeout=6, data=jsondata,
    ...
    def run(self, datas, jsondatas):
        ...
        allTask = [pool.submit(self.check, self.url, data, json.dumps(jsondata)) for data in datas for jsondata in
                   jsondatas]
