...
class SqlTest():
    ...
    def errorSQLInjection(self):
        ...
        if method == "post":
            ...
            try:
                if sslFlag == 1:
                    post_resp = requests.post(self.path, headers=self.header, data=post_data, proxies=self.proxy_data,
                ...
                if post_code == '415':
                    ...
                    try:
                        if sslFlag == 1:
                            post_json_resp = requests.post(self.path, data=post_json, headers=header,
                                                           proxies=self.proxy_data, timeout=10, verify=False)
                        else:
                            post_json_resp = requests.post(self.path, data=post_json, headers=header,
                                                           proxies=self.proxy_data, timeout=10)
                        ...
                        if post_json_code == '415':  # 如果状态码还是 415 就不管了 只负责json和普通的data
                            ...
                            for error in errors:
                                if error in post_json_text:
                                    ...
                                    self.error = 1
                                    try:
                                        DatabaseType(self.projectTag).insertSQLInfoIntoDB(self.api_id, self.from_js,
                                                                                          post_json, post_json_text)
                                    except Exception as e:
                                        pass
                    except Exception as e:
                        pass
                    ...
                    for error in errors:
                        if error in post_text:
                            ...
                            self.error = 1
                            try:
                                DatabaseType(self.projectTag).insertSQLInfoIntoDB(self.api_id, self.from_js, post_data,
                                                                                  post_text)
                            except Exception as e:
                                pass
            except Exception as e:
            ...
            try:
                if sslFlag == 1:
                    ...
                    get_resp2 = requests.get(url2, headers=self.header, proxies=self.proxy_data, timeout=10,
                                             verify=False)
                    get_resp_default = requests.get(url_default, headers=self.header, proxies=self.proxy_data,
                                                    timeout=10, verify=False)
                else:
                    get_resp1 = requests.get(url1, headers=self.header, proxies=self.proxy_data, timeout=10)
                    get_resp2 = requests.get(url2, headers=self.header, proxies=self.proxy_data, timeout=10)
                ...
                if (get_resp1_len == get_resp_default) and (get_resp2_len != get_resp_default):
                    #print("疑似存在sql布尔盲注")
                    self.boolen = 1
                    DatabaseType(self.projectTag).insertSQLInfoIntoDB(self.api_id, self.from_js, url_default,
                                                                      get_resp1.text)
            except Exception as e:
                pass
        ...
        if method == "post":
            ...
            try:
                ...
                if post_code == '415':
                    ...
                    if sslFlag == 1:
                        post_json_len1 = len(
                            requests.post(self.path, headers=header, proxies=self.proxy_data, data=post_json1,
                                          verify=False).text)
                        post_json_len2 = len(
                            requests.post(self.path, headers=header, proxies=self.proxy_data, data=post_json2,
                                          verify=False).text)
                        post_json_defaule_len = len(
                            requests.post(self.path, headers=header, proxies=self.proxy_data, data=post_json_default,
                                          verify=False).text)
                        post_resp_code = requests.post(self.path, headers=header, proxies=self.proxy_data,
                                                       data=post_json1, verify=False).status_code
                        post_resp_text = requests.post(self.path, headers=header, proxies=self.proxy_data,
                                                       data=post_json1, verify=False).text
                    else:
                        post_json_len1 = len(
                            requests.post(self.path, headers=header, proxies=self.proxy_data, data=post_json1).text)
                        post_json_len2 = len(
                            requests.post(self.path, headers=header, proxies=self.proxy_data, data=post_json2).text)
                        post_json_defaule_len = len(requests.post(self.path, headers=header, proxies=self.proxy_data,
                    ...
                    if sslFlag == 1:
                        post_len1 = len(
                            requests.post(self.path, headers=self.header, data=post_data1, proxies=self.proxy_data,
                                          verify=False).text)
                        post_len2 = len(
                            requests.post(self.path, headers=self.header, data=post_data2, proxies=self.proxy_data,
                                          verify=False).text)
                        post_len_default = len(requests.post(self.path, headers=self.header, data=post_data_default,
                                                             proxies=self.proxy_data, verify=False).text)
                    else:
                        post_len1 = len(requests.post(self.path, headers=self.header, data=post_data1,
                                                      proxies=self.proxy_data).text)
                        post_len2 = len(requests.post(self.path, headers=self.header, data=post_data2,
                                                      proxies=self.proxy_data).text)
                        post_len_default = len(requests.post(self.path, headers=self.header, data=post_data_default,
                                                             proxies=self.proxy_data).text)
                    if (post_len1 == post_len_default) and (post_len2 != post_len_default):
                        ...
                        try:
                            DatabaseType(self.projectTag).insertSQLInfoIntoDB(self.api_id, self.from_js,
                                                                              post_data_default, post_resp.text)
                        except Exception as e:
                            pass
    ...
    def timeSQLInjction(self):
        ...
        method = datas['type']
        if sslFlag == 1:
            default_time = requests.get(self.path, headers=self.header, proxies=self.proxy_data,
        ...
        if method == "post":
            ...
            try:
                ...
                if code == '415':
                    ...
                    if sslFlag == 1:
                        post_json_resp = requests.post(self.path, headers=header, data=post_json,
                                                       proxies=self.proxy_data, verify=False)
                    else:
                        post_json_resp = requests.post(self.path, headers=header, data=post_json,
                    ...
                    if json_code == '415':
                        ...
                        self.time = 1
                        try:
                            DatabaseType(self.projectTag).insertSQLInfoIntoDB(self.api_id, self.from_js, post_json,
                                                                              post_json_resp.text)
                        except Exception as e:
                            pass
