...
def is_not_matched(string):
    regex = re.compile(
        r"^(?!\./|.*application|DD/MM/YYYY|/sts/|/sa/|MM/DD/YYYY|/sa/|\.(js.*|html.*|txs.*|tx.*|jsessionid=.*)|.*audio|.*video|/$|/sbp/|//|.*text|.*image|[\u4E00-\u9FA5]|/.$|.*YYYY|^YYYY).*",
        re.IGNORECASE)
    return not bool(regex.match(string))
class Apicollect():
    ...
    def apiCollect(self, filePath):
        with open(filePath, "r", errors="ignore") as jsPath:
            apiStr = jsPath.read()
    ...
    def getBaseurl(self, filePath):
        ...
        if baseURL == None:
            ...
            with open(filePath, "r", errors="ignore") as js_path:
                ...
                for baseurlRegx in self.baseUrlRegxs:
                    ...
                    for baseurlPath in baseurLists:
                        if baseurlPath != '' and '/' in baseurlPath and baseurlPath != "/" and len(
                                baseurlPath) > 3 and len(
                            baseurlPath) < 20:
                            for apiExt in self.apiExts.split(","):
    ...
    def apiComplete(self):
        ...
        for baseurl in self.baseUrlPaths:
            for apiPath in self.apiPaths:
                if not is_not_matched(apiPath.split('§§§')[0]):
                    if baseurl == "/":
    ...
    def apireCoverStart(self):
        ...
        if len(self.apiPaths) < 30:  # 提取结果过少时暴力破解
            ...
            self.log.info(
                Utils().tellTime() + Utils().getMyWord("{total_api_1}") + str(len(self.apiPaths)) + Utils().getMyWord(
                    "{total_api_2}"))
