    ...
    def check(self, url, options):
        ...
        try:
            if self.mode == 1:
                try:
                    if sslFlag == 1:
                        code = str(s.get(url, headers=headers, timeout=6, proxies=self.proxy_data,
                                         verify=False).status_code)  # 正常的返回code是int类型
                    else:
            ...
            if self.mode == 2:
                ...
                try:
                    if sslFlag == 1:
                        response = str(
                            s.get(url, headers=headers, timeout=6, proxies=self.proxy_data, verify=False).headers)
                    else:
