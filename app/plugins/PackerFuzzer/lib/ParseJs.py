...
class ParseJs():  # 获取js进行提取
    ...
    def requestUrl(self):
        ...
        demo = requests.get(url=url, headers=headers, verify=False).text
    ...
    def dealJs(self, js_paths):  # 生成js绝对路径
        ...
        for jsPath in js_paths:  # 路径处理多种情况./ ../ / http
            if jsPath[:2] == "./":
                ...
                jsRealPath = baseUrl + jsPath
                self.jsRealPaths.append(jsRealPath)
        self.log.info(Utils().tellTime() + Utils().getMyWord("{pares_js_fini_1}") + str(
            len(self.jsRealPaths)) + Utils().getMyWord("{pares_js_fini_2}"))
        domain = res.netloc
        if ":" in domain:
