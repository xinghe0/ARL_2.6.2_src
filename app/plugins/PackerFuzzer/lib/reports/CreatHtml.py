            ...
            if vuln_info[3] == "unAuth" and vuln_info[4] == 1:
                ...
                for api_info in api_infos:
                    ...
                    for js_path in js_paths:
                        ...
                        tr_api_list = tr_api_list.replace("{vuln_url_type}", vuln_url_type)
                        tr_api_list = tr_api_list.replace("{vuln_length}", str(api_length_info))
                        tr_api_list = tr_api_list.replace("{vuln_risk}",
                                                          Utils().getMyWord("{r_l_l}") + "，" + Utils().getMyWord(
                                                              "{r_vuln_maybe}"))
                        tr_api_list = tr_api_list.replace("{vuln_id}", "vuln_" + str(creat_vuln_num))
                        self.creat_api_num = self.creat_api_num + 1
                        creat_vuln_num = creat_vuln_num + 1
...
</div>""" % (Utils().getMyWord("{vuln_info}"), Utils().getMyWord("{vuln_type}"), Utils().getMyWord("{unauth_vuln}"),
             Utils().getMyWord("{vuln_level}"), Utils().getMyWord("{vuln_path}"), Utils().getMyWord("{r_api_js}"),
             ...
             Utils().getMyWord("{r_api_res}"))
                    ...
                    for js_path in js_paths:
                        if vuln_info[6] == None:
                            ...
                            vuln_res = vuln_info[6]
                        self.creat_vuln_num = self.creat_vuln_num + 1
                        unauth_info_html = unauth_info_html.replace("{vuln_risk}", Utils().getMyWord(
                            "{r_l_l}") + "，" + Utils().getMyWord("{r_vuln_maybe}"))
                        unauth_info_html = unauth_info_html.replace("{vuln_url}", api_info[1])
                        unauth_info_html = unauth_info_html.replace("{vuln_js}", js_path[0])
                        unauth_info_html = unauth_info_html.replace("{vuln_res}", vuln_res)
                        whole_html = whole_html + unauth_info_html
...
</div>""" % (Utils().getMyWord("{vuln_info}"), Utils().getMyWord("{vuln_type}"), Utils().getMyWord("{info_vuln}"),
             Utils().getMyWord("{vuln_level}"), Utils().getMyWord("{r_js_des}"), Utils().getMyWord("{r_api_js}"),
             Utils().getMyWord("{r_js_detial}"), Utils().getMyWord("{related_frag}"))
...
</div>""" % (Utils().getMyWord("{vuln_info}"), Utils().getMyWord("{vuln_type}"), Utils().getMyWord("{passWord_vuln}"),
             Utils().getMyWord("{vuln_level}"), Utils().getMyWord("{vuln_path}"), Utils().getMyWord("{r_api_js}"),
             Utils().getMyWord("{request_info}"), Utils().getMyWord("{r_api_res}"))
...
</div>""" % (Utils().getMyWord("{vuln_info}"), Utils().getMyWord("{vuln_type}"), Utils().getMyWord("{CORS_vuln}"),
             Utils().getMyWord("{vuln_level}"), Utils().getMyWord("{vuln_path}"), Utils().getMyWord("{request_head}"),
             Utils().getMyWord("{response_head}"))
...
<a id="vuln_unauth" class="anchor"></a>
        ...
        Utils().getMyWord("{unauth_vuln}"), Utils().getMyWord("{api_name}"), Utils().getMyWord("{vuln_path}"),
        Utils().getMyWord("{r_type}"), Utils().getMyWord("{vuln_level}"), Utils().getMyWord("{api_length}"))
...
<a id="vuln_info" class="anchor"></a>
        ...
        Utils().getMyWord("{info_vuln}"), Utils().getMyWord("{info_vuln_type}"), Utils().getMyWord("{r_api_js}"),
        Utils().getMyWord("{r_js_detial}"), Utils().getMyWord("{vuln_level}"))
        ...
        <a id="vuln_password" class="anchor"></a>
            ...
            Utils().getMyWord("{passWord_vuln}"), Utils().getMyWord("{api_name}"), Utils().getMyWord("{vuln_path}"),
            Utils().getMyWord("{r_type}"), Utils().getMyWord("{vuln_level}"))
        ...
        <a id="vuln_cors" class="anchor"></a>
            ...
            Utils().getMyWord("{CORS_vuln}"), Utils().getMyWord("{web_addr}"), "Payload",
            Utils().getMyWord("{vuln_level}"))
...
<a id="vuln_bac" class="anchor"></a>
        ...
        Utils().getMyWord("{BAC_vuln}"), Utils().getMyWord("{api_name}"), Utils().getMyWord("{vuln_path}"),
        Utils().getMyWord("{r_type}"), Utils().getMyWord("{vuln_level}"))
...
<a id="vuln_upload" class="anchor"></a>
        ...
        Utils().getMyWord("{upload_vuln}"), Utils().getMyWord("{api_name}"), Utils().getMyWord("{vuln_path}"),
        Utils().getMyWord("{r_type}"), Utils().getMyWord("{vuln_level}"))
...
<a id="vuln_sql" class="anchor"></a>
        ...
        Utils().getMyWord("{sql_vuln}"), Utils().getMyWord("{api_name}"), Utils().getMyWord("{vuln_path}"),
        Utils().getMyWord("{r_type}"), Utils().getMyWord("{vuln_level}"))
        try:
    ...
    def api_detail_html(self):
        ...
        for api_info in api_infos:
            ...
            Utils().getMyWord("{api_detail}"), Utils().getMyWord("{api_name}"), Utils().getMyWord("{r_type}"),
            Utils().getMyWord("{r_api_addr}"), Utils().getMyWord("{r_api_js}"), Utils().getMyWord("{r_api_res}"))
    ...
    def report_html(self):
        ...
        Utils().getMyWord("{scaned_plat}"), Utils().getMyWord("{para_value}"), Utils().getMyWord("{scan_method}"),
        Utils().getMyWord("{scan_time}"), Utils().getMyWord("{s}"), Utils().getMyWord("{use}"),
        Utils().getMyWord("{ip_address}"), Utils().getMyWord("{co_discovery}"), Utils().getMyWord("{effective_api}"),
        Utils().getMyWord("{co_discovery}"), Utils().getMyWord("{effective_js}"), Utils().getMyWord("{co_discovery}"),
        Utils().getMyWord("{effective_vuln}"), Utils().getMyWord("{r_l_h}"), Utils().getMyWord("{ge}"),
        Utils().getMyWord("{r_l_m}"), Utils().getMyWord("{ge}"), Utils().getMyWord("{r_l_l}"),
        Utils().getMyWord("{ge}"), Utils().getMyWord("{extra_cookies}"), Utils().getMyWord("{extra_head}"),
        Utils().getMyWord("{vuln_total_level}"))
        ...
        if len(num_auth) != 0:
            vuln_list = vuln_list + "◆ " + Utils().getMyWord("{vuln_unauth_num}") + str(
                len(num_auth)) + Utils().getMyWord("{ge}") + "\n"
        sql = "select id from vuln where type='CORS'"
        cursor.execute(sql)
        num_cors = cursor.fetchall()
        if len(num_cors) != 0:
            vuln_list = vuln_list + "◆ " + Utils().getMyWord("{vuln_cors_num}") + str(
                len(num_cors)) + Utils().getMyWord("{ge}") + "\n"
        sql = "select id from vuln where type='INFO'"
        cursor.execute(sql)
        num_info = cursor.fetchall()
        if len(num_info) != 0:
            vuln_list = vuln_list + "◆ " + Utils().getMyWord("{vuln_info_num}") + str(
                len(num_info)) + Utils().getMyWord("{ge}") + "\n"
        sql = "select id from vuln where type='passWord'"
        cursor.execute(sql)
        num_passWord = cursor.fetchall()
        if len(num_passWord) != 0:
            vuln_list = vuln_list + "◆ " + Utils().getMyWord("{vuln_passWord_num}") + str(
                len(num_passWord)) + Utils().getMyWord("{ge}") + "\n"
        sql = "select id from vuln where type='BAC'"
        cursor.execute(sql)
        num_BAC = cursor.fetchall()
        if len(num_BAC) != 0:
            vuln_list = vuln_list + "◆ " + Utils().getMyWord("{vuln_BAC_num}") + str(len(num_BAC)) + Utils().getMyWord(
                "{ge}") + "\n"
        ...
        if len(num_upload) != 0:
            vuln_list = vuln_list + "◆ " + Utils().getMyWord("{vuln_upload_num}") + str(
                len(num_upload)) + Utils().getMyWord("{ge}") + "\n"
...
<li><h5>%s</h5></li>
1. %s<br>
2. %s<br><br>""" % (Utils().getMyWord("{r_sug_unauth_1}"), Utils().getMyWord("{r_sug_unauth_2}"),
...
2. %s<br>
3. %s<br><br>
        """ % (Utils().getMyWord("{r_sug_upload_1}"), Utils().getMyWord("{r_sug_upload_2}"),
               Utils().getMyWord("{r_sug_upload_3}"), Utils().getMyWord("{r_sug_upload_4}"))
        password_str = """
<a id="sug_password" class="anchor"></a>
<li><h5>%s</h5></li>
1. %s<br>
2. %s<br><br>
        """ % (Utils().getMyWord("{r_sug_password_1}"), Utils().getMyWord("{r_sug_password_2}"),
...
4. %s<br>
5. %s
        """ % (Utils().getMyWord("{r_sug_g_1}"), Utils().getMyWord("{r_sug_g_2}"), Utils().getMyWord("{r_sug_g_3}"),
               Utils().getMyWord("{r_sug_g_4}"), Utils().getMyWord("{r_sug_g_5}"), Utils().getMyWord("{r_sug_g_6}"))
        unauth_nav_str = """
<li class="nav-item">
...
<li class="nav-item">
  <a href="#sug_total" class="nav-link">
    ...
    def CreatMe(self):
        ...
        with open("doc" + os.sep + "template" + os.sep + "html" + os.sep + docLang + ".html", "r",
                  errors="ignore") as html:
            html_str = html.read()
            new_html_str = CreatHtml(self.projectTag, self.html_filepath).HtmlReplace(html_str)
            with open(self.html_filepath, "w", errors="ignore") as new_html:
