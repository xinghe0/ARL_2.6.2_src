...
class Config(object):
    ...
    MONGO_DB = 'arl'
    MONGO_URL = 'mongodb://arluser:admin@127.0.0.1:27017/'

    TMP_PATH = os.path.join(basedir, 'tmp')
    TMPPF_PATH = os.path.join(basedir, '../tmp')
    TMPlogs_PATH = os.path.join(basedir, '../logs')
    if not os.path.exists(TMP_PATH):
        os.mkdir(TMP_PATH)
    NUCLEI_BIN = os.path.join(basedir, 'tools/nuclei')
    WHI_BIN = os.path.join(basedir, 'tools/wih')
    NCRACK_BIN = os.path.join(basedir, 'tools/ncrack')
    XRAY_BIN = os.path.join(basedir, 'tools/xray_scan/xray')
    RAD_BIN = os.path.join(basedir, 'tools/rad_scan/rad')
    ...
    PHANTOMJS = os.path.join(basedir, 'tools/phantomjs')
    ...
    JSKEY_PATH = os.path.join(basedir, 'plugins/FindJsInfo/lib/JsKey.txt')
    TMPone_PATH = os.path.join(basedir, 'plugins/OneForAll/results')
    PackerFuzzer_PATH = os.path.join(basedir, 'plugins/PackerFuzzer/PackerFuzzer.py')
    PFresult_PATH = os.path.join(basedir, 'plugins/PackerFuzzer/result')
    PFtmp_PATH = os.path.join(basedir, 'plugins/PackerFuzzer/')
    PFlog_PATH = os.path.join(basedir, 'plugins/PackerFuzzer/')
    PFini_PATH = os.path.join(basedir, 'plugins/PackerFuzzer/doc/lang.ini')
    PFconfig_PATH = os.path.join(basedir, 'plugins/PackerFuzzer/config.ini')
    PATHKEY_PATH = os.path.join(basedir, 'plugins/FindJsInfo/lib/PathKey.txt')
    JSAPI_PATH = os.path.join(basedir, 'plugins/FindJsInfo/js_api.txt')
    SECRET_PATH = os.path.join(basedir, 'plugins/FindJsInfo/Secret.txt')
    ...
    GEOIP_ASN = os.path.join(basedir, 'dicts/GeoLite2-ASN.mmdb')
    GEOIP_CITY = os.path.join(basedir, 'dicts/GeoLite2-City.mmdb')
    ...
    AUTH = True
    API_KEY = "4c58b59dc2c779cc554986fc0848272b"
...
try:
    with open(os.path.join(basedir, 'arlconfig.yaml')) as f:
    ...
    # *** 获取index环境配置 ***
    if y.get("ISDOCKER"):
        if y["ISDOCKER"].get("KEY"):
            Config.ISDOCKER_KEY = y["ISDOCKER"]["KEY"]
    ...
    print("Parse arlconfig.yaml error {}".format(e))
    sys.exit(-1)
